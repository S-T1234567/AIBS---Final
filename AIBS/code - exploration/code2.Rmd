...
svm_wflow <- recipe(emotion ~ ., data = english_train) %>% 
...
english_bs <- bootstraps(english_train, times = 10)
...
```{r}
# The last fit
final_fit <- last_fit(object = final_wflow, split = english_split)
...
english_svm_model <- final_fit %>% 
  extract_workflow()

# Save the workflow
library(here)
saveRDS(english_svm_model, "english_svm_model.rds")
```
