...
cat("Training cases: ", nrow(spanish_train), "\n",
    "Test cases: ", nrow(spanish_test), sep = "")
...
multireg_fit <- multireg_spec %>% 
  fit(emotion ~ ., data = spanish_train)
...
```{r}
spanish_results <- spanish_test %>% select(emotion) %>% 
  bind_cols(multireg_fit %>% 
              predict(new_data = spanish_test)) %>% 
  bind_cols(multireg_fit %>% 
              predict(new_data = spanish_test, type = "prob"))
...
svm_wflow <- recipe(emotion ~ ., data = spanish_train) %>% 
...
spanish_bs <- bootstraps(spanish_train, times = 10)
...
```{r}
# The last fit
final_fit <- last_fit(object = final_wflow, split = spanish_split)
...
spanish_svm_model <- final_fit %>% 
  extract_workflow()

# Save the workflow
library(here)
saveRDS(spanish_svm_model, "spanish_svm_model.rds")
```
...
```{r}
loaded_psvm_model <- readRDS("spanish_svm_model.rds") 
```
